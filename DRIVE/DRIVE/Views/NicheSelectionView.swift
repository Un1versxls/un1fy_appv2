...
struct NicheSelectionView: View {
    ...
    private var continueButton: some View {
        ...
        .padding(.top, DriveSpacing.base)
            ...
            .navigationBarHidden(true)
            .alert("Selection Required", isPresented: $showingValidationError) {
                Button("OK", role: .cancel) {}
            } message: {
                Text("Please select at least \(minSelections) and up to \(maxSelections) niches to continue.")
            }
        }
    }
    ...
    private var selectionCounter: some View {
        HStack {
            Text("\(selectedNiches.count)/\(maxSelections) selected")
                .font(.driveCallout)
                .foregroundColor(.driveTextSecondary)
    ...
    private var continueButton: some View {
        VStack(spacing: DriveSpacing.base) {
            GradientButton(
                "Continue",
                icon: "arrow.right",
                isEnabled: selectedNiches.count >= minSelections && selectedNiches.count <= maxSelections
            ) {
                saveSelections()
            }
            .padding(.horizontal, DriveSpacing.base)
            
            if selectedNiches.count < minSelections {
                Text("Select at least \(minSelections) niche to continue")
                    .font(.driveCaption)
                    .foregroundColor(.driveTextTertiary)
            }
        }
    ...
    private func toggleSelection(_ niche: Niche) {
        withAnimation(DriveAnimations.standard) {
    ...
    private func saveSelections() {
        ...
        let niches = Niche.allNiches.filter { selectedNiches.contains($0.id) }
        appState.selectNiches(niches)
